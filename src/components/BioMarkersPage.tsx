...
export default function BioMarkersPage({ memberId, readOnly = false }: { memberId?: string; readOnly?: boolean } = {}) {
  ...
  return (
    <div className={styles.biomarkerShell} style={{ display: 'flex', flexDirection: 'column', gap: 18, color: 'var(--text-primary)' }}>
      ...
      {!readOnly && <form onSubmit={submit} className={styles.responsiveFormGrid} style={{ ...panel, display: 'grid', gridTemplateColumns: 'repeat(4, minmax(160px, 1fr)) auto', gap: 12, alignItems: 'end' }}>
        ...
        <button className={styles.biomarkerUploadButton} disabled={saving} style={{ minHeight: 40, padding: '0 16px', borderRadius: 'var(--radius-md)', background: 'var(--accent)', color: '#fff', fontWeight: 900, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      ...
      {loading ? <div style={panel}>Loading BioMarkers...</div> : (
        <>
          <div className={styles.responsiveFourCol} style={{ display: 'grid', gridTemplateColumns: '1.5fr .8fr 2fr .9fr', gap: 14 }}>
            <div className={styles.biomarkerPanel} style={panel}>
              <h3 style={{ margin: '0 0 18px', fontSize: 18 }}>{latest?.report_type || 'Blood'} Report Summary</h3>
              <div className={styles.biomarkerSummaryGrid} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div className={styles.biomarkerPanel} style={{ ...panel, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              ...
              <div className={styles.biomarkerScoreRing} style={{ width: 154, height: 154, margin: '0 auto 14px', borderRadius: '50%', display: 'grid', placeItems: 'center', background: `conic-gradient(${scoreColor} ${score * 3.6}deg, rgba(148,163,184,0.18) 0deg)`, boxShadow: `0 0 34px ${scoreColor}22`, position: 'relative' }}>
            ...
            <div className={styles.biomarkerPanel} style={panel}>
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                {comparison.length ? comparison.map((c: any) => {
                  ...
                  return <div key={c.parameter} className={`${styles.responsiveThreeCol} ${styles.biomarkerComparisonRow}`} style={{ display: 'grid', gridTemplateColumns: '150px 1fr 80px', gap: 10, alignItems: 'center' }}>
          ...
          <div className={styles.responsiveTwoCol} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 14 }}>
            <div className={styles.biomarkerPanel} style={panel}>
              ...
              <div className={styles.scrollTable}>
                <table className={styles.biomarkerTable} style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  ...
                  <tbody>
                    {reports.map((r: any) => <tr key={r.id}>
                      <td data-label="Report Date" style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{safeDate(r.report_date || r.created_at)}</td>
                      <td data-label="Lab Name" style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{r.lab_name || '-'}</td>
                      <td data-label="Report File" style={{ padding: 8, borderBottom: '1px solid var(--border)', color: 'var(--accent)' }}><FileText size={14} /> {r.file_name || 'Report'}</td>
                      <td data-label="Health Score" style={{ padding: 8, borderBottom: '1px solid var(--border)' }}><strong style={{ color: 'var(--success)' }}>{r.health_score || 0}</strong> / 100</td>
                      <td data-label="Status" style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>{statusBadge(r.status)}</td>
                      <td data-label="Action" style={{ padding: 8, borderBottom: '1px solid var(--border)' }}>
              ...
              <div className={styles.scrollTable}>
                <table className={styles.biomarkerTable} style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  ...
                  <tbody>{latestValues.map((v: any) => <tr key={v.id}>
                    <td data-label="Parameter" style={{ padding: 6, borderBottom: '1px solid var(--border)' }}>{v.parameter_name}</td>
                    <td data-label="Result" style={{ padding: 6, borderBottom: '1px solid var(--border)' }}>{v.value ?? v.value_text ?? '-'} {v.unit || ''}</td>
                    <td data-label="Reference Range" style={{ padding: 6, borderBottom: '1px solid var(--border)' }}>{v.reference_range || '-'}</td>
                    <td data-label="Status" style={{ padding: 6, borderBottom: '1px solid var(--border)' }}>{statusBadge(v.status)}</td>
                    <td data-label="Insight" style={{ padding: 6, borderBottom: '1px solid var(--border)', color: 'var(--text-secondary)' }}>{v.short_insight || v.recommended_action || '-'}</td>
                  </tr>)}</tbody>
                </table>
              </div>
            </div>
          ...
          <div className={styles.responsiveThreeCol} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
            ...
            <InfoList title="Resource Insights" items={latest?.trainer_friendly_insights || []} icon={<FileText size={16} color="var(--accent)" />} />
          </div>
          <div className={styles.responsiveThreeCol} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
